...
6. **Sandbox cleanup.** Task #7 from session `bgg-example` was paused at
   this step. Remove `prototype/sandbox/{annotate,charts,csv-poc,rate,
   tagcloud}` and update the `Layout` paragraph in root `CLAUDE.md` to
   point at `examples/bgg/` instead of `sandbox/`.

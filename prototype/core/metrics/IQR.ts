/** Verbatim port of legacy `@cocoon/plugin-distance/metrics/IQR.ts`.
 *  d3-scale → `./numeric.ts`; lodash → `core/lodash-lite.ts`. */
import { scaleLinear } from './numeric.ts';
import { isNil } from '../lodash-lite.ts';
import type { Metric } from './index.ts';
import { interquartileRange } from './statistics.ts';
...
export interface Config {
   ...
   * the midspread. Default is 1.5.
   */
  iqr?: number;

  /**
   * The score for values within the IQR. Default is 1.
   */
  reward?: number;

  /**
   * The score for values outside of the IQR. Default is 0.
   */
  penalty?: number;

  /**
   * The interpolation range, as a percentage of the total bounds.
   * Domain for valid values: [0, .5[. Default is .25.
   */
...
export const IQR: Metric<Config, Cache> = {
  cache(config, values, debug) {
    const filteredValues = values.filter(x => !isNil(x)) as number[];
    ...
    return {
      scale: scaleLinear()
        .domain(domain)
        .range(range)
        .clamp(true) as unknown as (x: number) => number,
    };
  },

  score(config, cache, value) {
    return isNil(value) ? null : cache.scale(value);
  },

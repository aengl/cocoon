...
Requires a running `cocoon serve <file> [--port N]`. Default target is `ws://localhost:4000`; override with `--core <ws-url|host:port|port>` or `COCOON_CORE`. Exit codes: `0` ok · `1` query failed · `2` no core reachable. (Inside this repo, prefix with `pnpm core …` from `prototype/`.)

...
- **The flow file and its `.env`/`.env.defaults` are watched; node *code* is not.** The watcher lives in `serve.ts` (one-shot `run` has no clients and must not arm it); a flow-file edit is a selective reload, a dotenv edit a full reset (env is a global input the resolver can't attribute to specific nodes). Node code is hot-swapped at execution time by the mtime-keyed resolver — no watcher, no `serve` restart.
